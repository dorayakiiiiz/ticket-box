              ...
              <div className="hidden md:flex items-center gap-3 group relative cursor-pointer">
                ...
                <div className="absolute right-0 top-full pt-2 w-32 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                  <div className="bg-[#111] border border-[#333] rounded-sm shadow-xl">
                    <button onClick={logout} className="w-full text-left px-4 py-2 text-[11px] md:text-xs font-semibold text-gray-400 hover:text-white hover:bg-[#222]">Đăng xuất</button>
                  </div>
                </div>
              </div>
            ) : (
              <button onClick={() => setLoginOpen(true)} className="hidden md:block text-[11px] md:text-sm font-semibold uppercase tracking-[0.12em] text-gray-400 hover:text-white transition-colors">Đăng nhập</button>
            )}
            <Link href="/" className="bg-[#CCFF00] text-black text-[10px] md:text-xs font-black uppercase tracking-[0.15em] px-4 py-2 hover:bg-white transition-colors">Mua vé</Link>
