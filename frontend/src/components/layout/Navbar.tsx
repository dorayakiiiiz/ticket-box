          ...
          <div className="flex items-center gap-3">
            {isAuthenticated && user ? (
              <div className="hidden md:flex items-center gap-3 group relative cursor-pointer">
                ...
                {/* Dropdown */}
                <div className="absolute right-0 top-full pt-2 w-36 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                  <div className="bg-[#111] border border-[#333] rounded-sm shadow-xl">
                    {user.role === "ORGANIZER" && (
                      <Link
                        href="/admin"
                        className="block w-full text-left px-4 py-2 text-[11px] font-semibold text-gray-400 hover:text-[#CCFF00] hover:bg-[#222] border-b border-[#333]"
                      >
                        Quản trị
                      </Link>
                    )}
                    <button
                      onClick={logout}
                      className="w-full text-left px-4 py-2 text-[11px] font-semibold text-gray-400 hover:text-white hover:bg-[#222]"
                    >
                      Đăng xuất
                    </button>
                  </div>
            ...
            <Link
              href="/"
              className="bg-[#CCFF00] text-black text-[10px] md:text-xs font-black uppercase tracking-[0.15em] px-4 py-2 hover:bg-white transition-colors"
            >
              Mua vé
            </Link>
            <button
