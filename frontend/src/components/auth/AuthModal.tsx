...
export default function AuthModal({ onClose }: { onClose: () => void }) {
  ...
  const [step, setStep] = useState<"form" | "otp">("form"); // For register flow
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {step === 'form' ? (
            <>
              ...
              {tab === "login" && <div className="text-right"><a href="#" className="text-[11px] md:text-xs text-[#CCFF00] hover:underline">Quên mật khẩu?</a></div>}
              ...
              <div className="flex gap-2 justify-center">
                {otp.map((digit, i) => (
                  <input
                    ...
                    className="w-10 h-12 text-center bg-[#111] border border-[#333] text-white text-lg font-bold outline-none focus:border-[#CCFF00]/50"
                  />
                ))}
              </div>
