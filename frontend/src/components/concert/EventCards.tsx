  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-2 left-3 text-[9px] font-mono tracking-[0.15em] text-[#CCFF00] uppercase">
          {dateStr}
        </div>
      </div>
      <div className="p-3">
        <h3
          style={D}
          className="text-sm font-bold uppercase tracking-wide text-white line-clamp-1"
        >
          {concert.name}
        </h3>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-[#CCFF00] hover:text-black hover:bg-[#CCFF00] px-2 py-0.5 transition-all border border-[#CCFF00]/30 hover:border-[#CCFF00]">
            Mua vé
          </span>
        </div>
        <SoldBar pct={sold} />
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5">
        <div className="text-[10px] font-mono tracking-[0.2em] text-[#CCFF00] uppercase mb-2">
        ...
        <h2
          style={D}
          className="text-3xl font-black uppercase italic tracking-tight leading-none text-white mb-1"
        >
          {concert.name}
        </h2>
        <p className="text-xs text-white/60 mb-3">{concert.subtitle}</p>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xl font-black text-white">{fmt(price)}đ</div>
          </div>
          <span className="bg-[#CCFF00] text-black text-xs font-black uppercase tracking-widest px-4 py-2 hover:bg-white transition-colors">
            Mua vé →
          </span>
        </div>
        <SoldBar pct={sold} />
    ...
    >
      <div style={{ aspectRatio: "3/4" }} className="relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <div className="text-[9px] font-mono tracking-[0.15em] text-[#CCFF00] uppercase mb-1">
            {new Date(concert.date).toLocaleDateString("vi-VN", {
              day: "2-digit",
              month: "short",
            })}
          ...
          <h3
            style={D}
            className="text-sm font-black uppercase italic text-white leading-tight mb-0.5"
          >
            {concert.name}
          </h3>
          <p className="text-[10px] text-white/50">{concert.city}</p>
          <div className="mt-1">
            <span className="text-xs font-bold text-white">{fmt(price)}đ</span>
          </div>
          <SoldBar pct={sold} />
        </div>
      </div>
