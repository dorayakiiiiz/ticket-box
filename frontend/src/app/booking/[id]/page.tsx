    ...
    <div className="min-h-screen flex flex-col">
      <div className="border-b border-[#222] bg-[#0a0a0a] px-6 py-4 flex items-center justify-between">
        ...
        <div className="text-center">
          ...
          <div className="text-[10px] text-gray-500">{event.date} · {event.venue}</div>
        </div>
        <div className="w-20" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row flex-1">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-x-4 gap-y-2">
            {zones.slice(0, 7).map(z => (
              <div key={z.id} className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-mono text-gray-400">{z.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full lg:w-80 border-l border-[#222] bg-[#0a0a0a] flex flex-col">
          <div className="p-5 border-b border-[#222]">
            <div className="text-[10px] font-mono tracking-[0.2em] text-gray-400 uppercase mb-1">{event.date} · {event.time}</div>
            <div style={D} className="text-xl font-black uppercase italic">{event.name}</div>
            <div className="flex items-center gap-1 text-[11px] text-gray-400 mt-1"><MapPin size={10} />{event.venue}</div>
          </div>

          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-1">
            <div className="text-[10px] font-mono tracking-[0.15em] text-gray-500 uppercase mb-2">Giá vé</div>
            {zones.map(z => (
              <button key={z.id} onClick={() => { setSel(z); setQty(1); }}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div><div className="text-xs font-bold">{z.name}</div><div className="text-[9px] text-gray-500">{z.type} · {z.available} vé</div></div>
                </div>
          ...
          {sel ? (
            <div className="p-5 border-t border-[#222]">
              ...
              <div className="text-[11px] text-gray-400 mb-4">{sel.type} · Còn {sel.available} vé</div>
              <div className="flex items-center gap-3 mb-4">
                <span className="text-[11px] text-gray-500 uppercase tracking-widest flex-1">Số lượng</span>
                <div className="flex items-center gap-2">
            ...
            <div className="p-5 border-t border-[#222] text-center text-[11px] text-gray-500">Chọn một khu vực để tiếp tục</div>
          )}
        </div>
      </div>
    </div>
