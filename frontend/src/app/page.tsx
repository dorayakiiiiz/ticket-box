    ...
    <>
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-12 pt-16 pb-8">
        <div className="flex items-end justify-between mb-6">
          ...
          <button className="hidden md:flex items-center gap-1 text-[11px] font-semibold text-gray-400 hover:text-[#CCFF00] uppercase tracking-widest transition-colors">Xem tất cả <ArrowRight size={12} /></button>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 mb-6" style={{ scrollbarWidth: "none" }}>
          {categories.map(c => (
            <button key={c} onClick={() => setCat(c)}
              className={`shrink-0 text-[10px] font-bold uppercase tracking-[0.15em] px-3 py-1.5 border transition-all ${cat === c ? "bg-[#CCFF00] text-black border-[#CCFF00]" : "text-gray-400 border-[#333] hover:border-[#CCFF00]/30 hover:text-white"}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="py-20 text-center text-gray-500 text-sm">Không có sự kiện nào cho danh mục này.</div>
        )}
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-12 py-8 border-t border-[#222]">
        ...
        {upcoming.map((ev, i) => (
          <Link href={`/concert/${ev.id}`} key={i} className="group flex items-center gap-4 md:gap-8 py-4 border-b border-[#222] hover:bg-[#111] transition-colors cursor-pointer px-3 -mx-3 block sm:flex">
            ...
            <span className="hidden md:block shrink-0 text-[9px] font-bold tracking-[0.15em] border border-[#333] text-gray-400 px-2 py-0.5">{ev.cat}</span>
            <div className="flex-1 min-w-0">
              <h3 style={D} className="font-black uppercase text-sm tracking-wide truncate group-hover:text-[#CCFF00] transition-colors">{ev.name}</h3>
              <div className="text-[11px] text-gray-500 mt-0.5">{ev.city}</div>
            </div>
            <div className="text-right shrink-0"><div className="text-sm font-bold">{ev.price}</div><div className="text-[10px] text-gray-500">Từ</div></div>
            <div className="hidden md:block shrink-0 border border-[#CCFF00]/30 text-[#CCFF00] text-[10px] font-black uppercase tracking-widest px-4 py-1.5 group-hover:bg-[#CCFF00] group-hover:text-black transition-all">Mua →</div>
          </Link>
        ))}
      </section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {[
            ...
            <button key={n} onClick={() => {
              ...
              <div style={D} className="text-sm font-bold uppercase tracking-wide text-white group-hover:text-[#CCFF00] transition-colors">{n}</div>
              <div className="text-[11px] text-gray-500 mt-0.5">{c} sự kiện</div>
            </button>
          ))}
        </div>
      </section>
