  ...
  return (
    <div className="min-h-screen">
      <div className="relative h-[52vh] overflow-hidden">
        ...
        <button onClick={() => router.back()} className="absolute top-6 left-6 flex items-center gap-2 text-[11px] md:text-xs font-semibold text-white/60 hover:text-white uppercase tracking-widest transition-colors">
        ...
        <div className="absolute bottom-6 left-6 md:left-12">
          ...
          <p className="text-sm md:text-base text-white/45 mt-2">{event.subtitle}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12">
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
              {[
                ...
                <div key={label} className="bg-[#0a0a0a] border border-[#222] p-4">
                  <div className="flex items-center gap-2 mb-2">{icon}<span className="text-[9px] md:text-[10px] font-mono uppercase tracking-widest text-gray-500">{label}</span></div>
            ...
            <div className="mb-10">
              ...
              <p className="text-sm md:text-base text-gray-400 leading-relaxed">{event.description}</p>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {event.artistList.map((a, i) => (
                  <div key={i} className="bg-[#0a0a0a] border border-[#222] p-4 flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm md:text-base font-bold">{a.name}</div>
                      <div className="text-[10px] md:text-xs text-gray-500">{a.role}</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {zones.slice(0, 6).map(z => (
                  <div key={z.id} className="flex items-center justify-between bg-[#0a0a0a] border border-[#222] px-4 py-3 hover:border-[#CCFF00]/20 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm md:text-base font-bold">{z.name}</div>
                        <div className="text-[10px] md:text-xs text-gray-500">{z.type} · Còn {z.available} vé</div>
                      </div>
                    </div>
                    <div className="text-sm md:text-base font-black">{fmt(z.price)}đ</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <div className="sticky top-20 bg-[#0a0a0a] border border-[#222] p-6">
              <div className="text-[10px] md:text-xs font-mono tracking-[0.2em] text-gray-500 uppercase mb-1">Vé từ</div>
