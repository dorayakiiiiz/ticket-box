          ...
          <div>
            ...
            <div className="flex flex-col gap-4 mb-8">
              {[
                ...
                <div key={label}>
                  <label className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-500 block mb-1.5">{label}</label>
                  <input type={type} value={val} onChange={e => set(e.target.value)} placeholder={ph}
                    className="w-full bg-[#111] border border-[#333] text-white text-sm px-4 py-3 outline-none focus:border-[#CCFF00]/50 placeholder-gray-600" />
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-3 mb-8">
              {[
                ...
                <button key={p.id} onClick={() => setPay(p.id)}
                  ...
                  <div><div className="text-sm font-bold text-white">{p.label}</div><div className="text-[11px] text-gray-500">{p.sub}</div></div>
                </button>
              ))}
            </div>
          ...
          <div>
            <div className="sticky top-20">
              <div className="bg-[#0a0a0a] border border-[#222] p-5 mb-3">
                <div className="text-[10px] font-mono tracking-[0.2em] text-gray-500 uppercase mb-3">Thông tin đặt vé</div>
                <div style={D} className="text-xl font-black uppercase italic mb-1">{event.name}</div>
                <div className="text-[11px] text-gray-400 mb-4">{event.date} · {event.time} · {event.venue}</div>
                <div className="border-t border-[#333] pt-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <div><div className="text-sm font-bold">{zone.name}</div><div className="text-[10px] text-gray-500">{zone.type} · x{qty}</div></div>
