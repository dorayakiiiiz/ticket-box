...
Frontend hiển thị sơ đồ chỗ ngồi dưới dạng ảnh SVG tĩnh (upload bởi ORGANIZER), kết hợp với overlay hiển thị số vé còn lại theo từng khu. Việc render được xử lý hoàn toàn ở phía client:

```mermaid
flowchart TB
    subgraph Data["Dữ liệu từ API"]
        API["GET /concerts/:id/availability"]
        TT["ticketTypes: name, colorCode, available, price"]
    end

    subgraph Render["Frontend Render"]
        IMG["Sơ đồ SVG tĩnh từ Supabase Storage"]
        OVL["Overlay: Badge số vé + màu khu"]
        MAP["CSS positioning map khu vực"]
    end

    subgraph UX["Trải nghiệm người dùng"]
        HOVER["Hover khu: hiển thị tooltip giá + số vé còn"]
        CLICK["Click khu: tự động chọn loại vé tương ứng"]
        COLOR["Màu sắc: colorCode từ TicketType entity"]
    end
...
Cách tiếp cận: hệ thống quản lý vé theo **khu vực** (GA, VIP, SVIP, CAT1, CAT2), không theo ghế đánh số. ORGANIZER upload ảnh sơ đồ tổng thể, Frontend sử dụng `colorCode` của từng TicketType (được cấu hình khi tạo loại vé) để hiển thị màu sắc tương ứng trên overlay. Số vé còn lại được lấy từ API Availability (cập nhật mỗi 5 giây qua SWR).
...
Khi ORGANIZER quyết định hủy concert (do bất khả kháng, thay đổi kế hoạch...), hệ thống cần xử lý các đơn hàng đang hoạt động và hoàn vé về Redis.
...
sequenceDiagram
    ...
    FE->>BE: PATCH /concerts/:id/cancel
    BE->>BE: JwtAuthGuard + RolesGuard (ORGANIZER)
    BE->>DB: SELECT concert WHERE id = ?

    alt Concert không tồn tại
        BE-->>FE: 404 Not Found
    else Concert đã bị hủy trước đó
        BE-->>FE: 409 "Concert đã được hủy"
    else Concert hợp lệ
        BE->>DB: BEGIN TRANSACTION
        BE->>DB: UPDATE concerts SET status = 'CANCELLED'

        BE->>DB: SELECT orders WHERE concert_id = ?<br/>AND status IN ('PENDING', 'PAID')
        DB-->>BE: Danh sách đơn hàng cần hủy

        loop Duyệt từng Order
            BE->>DB: UPDATE order SET status = 'CANCELLED'
            BE->>R: rollbackBooking(ticketTypeId, userId, quantity)
            alt Redis rollback thành công
                BE->>DB: UPDATE order SET isRefundedToRedis = true
            else Redis rollback thất bại
                BE->>DB: UPDATE order SET isRefundedToRedis = false
                Note right of DB: Self-healing sẽ bắt lại lần sau
            end
        end

        BE->>DB: COMMIT TRANSACTION
        BE-->>FE: 200 { status: 'CANCELLED', cancelledOrders: N }
    end
```

Lưu ý: Việc hoàn tiền (refund) qua cổng thanh toán thuộc phạm vi ngoài hệ thống (Out of Scope). Hệ thống chỉ hủy đơn hàng và hoàn vé về Redis; quy trình hoàn tiền được thực hiện thủ công bởi ban tổ chức.
...
### 5.5. Tại sao dùng `pdf-parse` thay vì các thư viện PDF khác?

| Tiêu chí | pdf-parse (Đã chọn) | pdfjs-dist (Mozilla) | pdf-lib |
|---|---|---|---|
| Mục đích chính | Trích xuất text từ PDF | Render PDF trong browser | Tạo/chỉnh sửa PDF |
| Môi trường | Node.js (backend) | Browser (cần polyfill cho Node) | Cả hai |
| API | Đơn giản — 1 hàm `pdfParse(buffer)` | Phức tạp — cần init worker, render từng trang | Không hỗ trợ trích xuất text |
| Kích thước | ~50KB (nhẹ) | ~2MB (nặng) | ~300KB |
| Phù hợp dự án | Có — chỉ cần trích text cho AI | Không — không cần render PDF | Không — không cần tạo/sửa PDF |

**Quyết định:** `pdf-parse`. Nhu cầu duy nhất là trích xuất nội dung text từ file PDF Press Kit để gửi cho AI sinh tiểu sử. `pdf-parse` cung cấp API đơn giản nhất (`pdfParse(buffer)` trả về object với `text`, `numpages`), nhẹ nhất (~50KB), và chạy native trên Node.js. `pdfjs-dist` mạnh hơn nhưng thiết kế cho browser rendering — quá phức tạp và nặng cho bài toán này. `pdf-lib` không hỗ trợ trích xuất text.
...
| 17  | ORGANIZER hủy concert có 50 đơn PAID và 10 đơn PENDING | Concert status chuyển CANCELLED. 60 đơn hàng chuyển CANCELLED. Redis hoàn vé cho tất cả. isRefundedToRedis = true          |
| 18  | ORGANIZER hủy concert đã bị hủy trước đó     | 409 "Concert đã được hủy"                                                                                                  |
| 19  | Xem trang concert có sơ đồ chỗ ngồi         | Ảnh SVG hiển thị với overlay màu sắc theo colorCode của từng TicketType. Số vé còn lại cập nhật mỗi 5 giây                  |
