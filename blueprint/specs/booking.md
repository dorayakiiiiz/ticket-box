...
flowchart TB
    subgraph Client["Frontend (Next.js)"]
        USER["Người dùng bấm Đặt Vé"]
        POLL["SWR Polling mỗi 2 giây"]
    end

    subgraph Guards["Chuỗi Guard Bảo Vệ"]
        G1["1. JwtAuthGuard"]
        G2["2. ThrottlerGuard"]
        G3["3. CaptchaGuard"]
    ...
    subgraph Cron["CronService - Mỗi 5 phút"]
        CANCEL["Hủy đơn PENDING quá 15 phút"]
...
sequenceDiagram
    ...
    Note over U,DB: Bước 0 — Chuẩn bị phía Client
    FE->>FE: Khi mount trang booking:<br/>1. Kiểm tra sessionStorage<br/>2. Nếu chưa có: sinh UUID (crypto.randomUUID)<br/>3. Lưu vào sessionStorage

    Note over U,DB: Bước 1 — Gửi Request Đặt Vé
    U->>FE: Chọn loại vé, số lượng, bấm "Đặt Vé"
    ...
    Note over BE: Lớp 1: JwtAuthGuard
    BE->>BE: Verify JWT từ Cookie, lấy userId

    Note over BE: Lớp 2: ThrottlerGuard
    BE->>R: INCR throttler:{userId}
    alt Vượt quá 3 request trong 1 giây
        BE-->>FE: 429 Too Many Requests
    end

    Note over BE: Lớp 3: CaptchaGuard
    BE->>CF: POST siteverify { token, secret }
    CF-->>BE: { success: true/false }
    alt Captcha thất bại
        BE-->>FE: 403 Forbidden
    end

    Note over BE: Lớp 4: IdempotencyGuard
    BE->>R: SET NX idempotency:{UUID} 'processing' EX 3600
    alt Key đã tồn tại (double-click)
        BE-->>FE: 409 "Yêu cầu trùng lặp"
    end

    Note over BE,DB: Bước 2 — Validate Nghiệp Vụ
    BE->>DB: SELECT ticketType WHERE id=? (JOIN concert)
    alt TicketType không tồn tại
        BE-->>FE: 404 "Loại vé không tồn tại"
    else Chưa đến thời gian mở bán
        BE-->>FE: 400 "Sự kiện chưa đến thời gian mở bán vé"
    end

    Note over BE,R: Bước 3 — Redis Lua Script (Atomic)
    ...
    Note right of R: Lua Script chạy đơn luồng, atomic:<br/>1. GET số vé user đã mua<br/>2. Kiểm tra hạn mức<br/>3. GET số vé còn lại<br/>4. Kiểm tra khả dụng<br/>5. DECRBY trừ vé<br/>6. INCRBY tăng counter user<br/>7. Trả về SUCCESS

    alt SOLD_OUT
        R-->>BE: 'SOLD_OUT'
        BE-->>FE: 400 "Rất tiếc, loại vé này đã hết!"
    else LIMIT_EXCEEDED
        R-->>BE: 'LIMIT_EXCEEDED'
        BE-->>FE: 400 "Bạn đã mua vượt quá giới hạn N vé!"
    else SUCCESS
        R-->>BE: 'SUCCESS'
    end

    Note over BE,Q: Bước 4 — Đẩy Job Vào Queue
...
sequenceDiagram
    ...
    alt Chưa hết số lần retry (attemptsMade < attempts)
        W->>W: KHÔNG trả vé Redis<br/>Giữ nguyên slot, BullMQ sẽ retry
...
```mermaid
sequenceDiagram
    participant CRON as CronService<br/>(mỗi 5 phút)
    participant R as Redis
    participant DB as PostgreSQL
    ...
    alt Không lấy được lock (server khác đang chạy)
        ...
        CRON->>DB: SELECT orders WHERE status=PENDING<br/>AND createdAt < (now - 15 phút)
        DB-->>CRON: Danh sách các đơn hết hạn
        ...
        DB-->>CRON: Danh sách đơn chưa hoàn vé
